...
//        for(int i = src.first-1; i < src.first + 2; i++){
//            for(int j = src.second - 1; j < src.second + 2; j++){
//                helper_board[i][j] = board[i][j];
//            }
//        }
        ...
        for(int i = src.first-1; i <  src.first + 2; i++){
            for(int j = src.second - 1; j < src.second + 2; j++){
                ...
                if(helper_board[i][j] && board[src.first][src.second]->getPlayer() == helper_board[i][j]->getPlayer() &&
                        (i-1 != src.first || j-1 != (src.second)))
                {
